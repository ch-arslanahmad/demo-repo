...
Fast forward Doesn't touch commits - just moves the branch pointer forward, no new commits created, no rewrititng, while rebase does the opposite, touches, moves, rewrites, even creates commits, as fast-forward is simply moving pointer forward while rebase is rewriting history.
